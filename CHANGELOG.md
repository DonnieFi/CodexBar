...
- Linux: restore Cursor usage through the signed-in app token, including the Grok Bot weekly allowance. Preserve manual-cookie precedence, keep explicit web mode independent of app credentials, and honor absolute `HOME` when resolving the Cursor database path. Thanks @DonnieFi for upstream PR #1908!
